...
# final files have structure like this 
# trainset[1:5]
...
datasets <- c(UStwitter, USblogs, USnews)
rm(con, UStwitter, USblogs, USnews)

## creating trainset and testset
set.seed(1234)
sampleline <- rbinom(length(datasets), 1, 0.60)
trainset <- datasets[as.logical(sampleline)]
testset <- datasets[!as.logical(sampleline)]

rm(sampleline, datasets)
